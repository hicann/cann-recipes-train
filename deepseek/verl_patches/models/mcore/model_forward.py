...
from verl.models.mcore.util import (postprocess_packed_seqs, preprocess_packed_seqs, recover_left_padding,
                                    remove_left_padding)


def convert_mask_for_npu_fa(original_mask):
    """
    Transform the `original_mask` to a format expected by `torch_npu.npu_fusion_attention`:
    1. Shape of mask is changed from [B, S] to [B, 1, S, S].
    2. The meaning of boolean mask differs:
       - In original mask, True indicates the positions of non-padding tokens.
       - In new mask, True indicates the positions which doesn't participate Q*K.T attn computation.
    """
    b, s = original_mask.shape

    causal_mask = torch.triu(torch.ones(s, s, dtype=torch.bool), diagonal=1).to(original_mask.device)

    padding_mask = ~original_mask.unsqueeze(2)
...
def gptmodel_forward(
    ...
    if pack_seqs:
        ...
        converted_mask = convert_mask_for_npu_fa(new_attention_mask)
        output = model(input_ids=new_input_ids, attention_mask=converted_mask, position_ids=new_position_ids)
