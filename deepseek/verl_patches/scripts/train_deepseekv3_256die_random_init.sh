...
export VLLM_ENABLE_FIX_ROUTE=1                  # 0:Turn off the fixed route of vLLM; 1:enable the fixed route of vLLM
...
if [ "$MASTER_ADDR" = "$CURRENT_IP" ]; then
  ...
  while true; do
      ...
      if [ "$device_count" -eq "$NNODES" ]; then
          ...
          ray status
          bash ./verl_patches/scripts/internal/train_grpo_deepseekv3_256die_random_init.sh
          break
      else
