...
本项目基于 **Qwen2.5-1.5B-Instruct** 模型，采用 **verl** 强化学习框架，在 **MATH-lighteval** 数学推理数据集上进行了训练。目标是提升小模型在复杂数学问题上的分步推理能力，使其能够生成逻辑严密且结果可验证的推理过程。本样例只需要单卡 Atlas A2 环境，帮助大家快速上手，使用昇腾 NPU 完成 RL 训练任务。
...
3. 下载项目源码并安装依赖的 python 库。
   ...
   # 安装依赖的python库
   cd cann-recipes-train/rl_train/qwen2_5/verl_npu_demo
...
3. 配置测试脚本
   ...
   - `path` ：测试模型存放的绝对路径，示例： `/home/cann-recipes-train/rl_train/qwen2_5/verl_npu_demo/verl/checkpoints/../../global_step_<训练步数>/actor/huggingface` 。
