# Adapted from
# https://github.com/volcengine/verl/blob/v0.4.0/examples/grpo_trainer/run_deepseek671b_math_megatron.sh
...
python3 -m verl.trainer.main_ppo_npu --config-path=config \
    ...
    actor_rollout_ref.model.gradient_checkpointing_kwargs.activations_checkpoint_granularity=full \
    actor_rollout_ref.model.gradient_checkpointing_kwargs.activations_checkpoint_method=block \
    actor_rollout_ref.model.gradient_checkpointing_kwargs.activations_checkpoint_num_layers=4 \
    ...
    trainer.project_name='verl_grpo_example_true_weight' \
