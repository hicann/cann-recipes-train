...
前置依赖：
vllm_ascend 0.9.1上在torchair_graph_config中提供了use_cached_graph和graph_batch_sizes的能力，支持提前配置多档位BS的图，并随着剩余Seq减少时自动匹配最小BS的图进行推理。
...
### 1.3 实验结果
我们在Atlas A3集群128卡环境上进行了如下实验，发现本方案开启后单轮推理耗时从6200s左右优化到约2300s，性能收益达57%~62%左右。
