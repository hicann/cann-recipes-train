...
NGPU=${NGPU:-"16"}
CONFIG_FILE=${CONFIG_FILE:-"torchtitan_npu/models/deepseek_v4/train_configs/deepseek_v4_285b_43layers_4k_128die.toml"}
TRAIN_FILE=${TRAIN_FILE:-"torchtitan_npu.entry"}
time=$(date +%Y%m%d%H%M)
logfile=dsv4_flash_128die_pretrain_${time}_node${NODE_RANK}_${LOCAL_HOST//./_}.log
mkdir -p logs
