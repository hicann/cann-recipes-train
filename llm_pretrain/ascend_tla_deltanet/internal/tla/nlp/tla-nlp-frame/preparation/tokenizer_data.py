...
def tokenize(
    ...
    if ctx_len is not None:
        input_ids = [
            ...
            for i in range(0, len(seq), ctx_len)
        ]
...
def preprocess(dataset: str, **kwargs: Any) -> None:
    config = PreprocessConfig(dataset=dataset, **kwargs)
    ...
    return_offsets = config.return_offsets
    """
    ...
    dataset = load_dataset(dataset, name=name, split=split, num_proc=num_proc)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max contiguous length to preserve (will not be split)"
    )
    parser.add_argument(
        "--return_offsets",
        action="store_true",
        help="Return cumulative offsets for concatenated inputs"
    )
