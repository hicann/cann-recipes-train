...
for arg in $args; do
  case "$arg" in
    *=*) eval "$arg" ;;
    *) echo "WARNING: ignoring malformed arg: $arg" ;;
  esac
done

echo "model:            ${model:=openai-community/gpt2-medium}"
echo "tokenizer:        ${tokenizer:=openai-community/gpt2-medium}"
...
# run.py loads tokenized data via load_from_disk(cache_dir); --dataset is metadata only.
if [ -z "$cache" ]; then
  echo "ERROR: cache is required (path to tokenized dataset from load_from_disk)."
  echo "  Example: cache=/path/to/tokenized"
  echo "  Prepare with: python ../preparation/tokenizer_data.py --dataset <raw> --output <cache> ..."
  exit 1
fi
if [ ! -d "$cache" ]; then
  echo "ERROR: cache directory does not exist: $cache"
  exit 1
fi
if [ ! -f "$cache/dataset_info.json" ] && [ ! -f "$cache/state.json" ]; then
  echo "ERROR: cache does not look like a datasets save_to_disk directory: $cache"
  echo "  Expected dataset_info.json or state.json under cache."
  exit 1
fi
