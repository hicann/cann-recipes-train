...
from attentions.deltanet import replace_attn_forward
...
def main():
    ...
    # 5. 配置学习率调度器 (Learning Rate Scheduler)
    ...
    trainer = Trainer(
        model=model,  # 要训练的模型
        args=args,  # 训练参数 (TrainingArguments 的实例)
        processing_class=tokenizer,  # 传入分词器，以便 Trainer 内部使用
        data_collator=data_collator,  # 数据整理器
        callbacks=[LogCallback()],  # 自定义的回调函数列表，这里是日志回调
